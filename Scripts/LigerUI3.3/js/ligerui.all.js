...
function setIframeHeight(iframe) {
	if (iframe) {
		...
		if (iframeWin.document.body) {
			var _contentHeight = iframeWin.document.documentElement.scrollHeight || iframeWin.document.body.scrollHeight;
			_contentHeight = _contentHeight + 20;
			$(iframe).height(_contentHeight);
			$(iframe).parent().height(_contentHeight);
		}
	}
};
/**
		...
		_render: function ()
		{
			...
			if (p.items) {
				$.each(p.items, function () {
					...
					var content = $('<div class="l-accordion-content"><ul></ul></div>');
					if (this.selected)
						$(".l-accordion-toggle", header).addClass("l-accordion-toggle-open");
					if (this.title) {
						$(".l-accordion-header-inner", header).html(this.title);
						//$(box).attr("title", "");
					}
					if (this.type) {
						switch (this.type) {
							case "shortcut":
								content.find("ul").ligerShortcut(this.editor);
								break;
							case "tree":
								content.find("ul").ligerTree(this.editor);
								break;
							default: content.find("ul").ligerShortcut(this.editor); break;
						}
					} else if (this.render) {
						//用于自定义渲染面板内容
						//参数（obj,data）
						this.render(content.find("ul"), this);
					}
					if (p.headHeight) header.height(p.headHeight).css({"line-height":p.headHeight+"px"});
					g.accordion.append(header);
					g.accordion.append(content);
					//$(box).before(header);
					//if (!$(box).hasClass("l-accordion-content")) $(box).addClass("l-accordion-content");

					if (this.icon) {
						header.addClass("l-accordion-header-hasicon");
						header.append('<i><img src="' + this.icon + '" /></i>');
					}
				});
			} else {
				//var selectedIndex = 0;
			...
			//160518 Andy Kang
            $(".l-accordion-header-inner", g.accordion).hover(function () {
                $(this).addClass("l-accordion-header-inner-over");
            }, function () {
                $(this).removeClass("l-accordion-header-inner-over");
            });
			...
			$(">.l-accordion-header", g.accordion).click(function () {
				...
				if (togglebtn.hasClass("l-accordion-toggle-close")) {
					...
					$(this).next(".l-accordion-content").slideUp(p.speed);
				}
				$(".l-accordion-header", g.accordion).removeClass("l-accordion-header-downfirst");
				$(".l-accordion-content:visible", g.accordion).next(".l-accordion-header:first").addClass("l-accordion-header-downfirst");
			});
...
(function ($)
{
	...
	$.ligerui.controls.CheckBox.ligerExtend($.ligerui.controls.Input, {
		...
		_setValue: function (value) {
			//修复 value 为 bool 类型，无法识别 是否选择   modify by Bill He 2016-05-06
		...
		setData: function (data)
		{
			...
			for (var i = 0; i < data.length; i++) {
				...
				if (!p.columns) {
					...
					if (p.showKeyCode)
						out.push("<td index='" + i + "' value='" + val + "' text='" + txt + "' align='left'>" + itemHtml + "-" + val + "</td></tr>");
					else
			...
			g.inputText.keyup(function (event) {
				...
				//this._acto = setTimeout(function () {
					...
					if (p.url) {
						...
						g.setParm(p.textField, g.inputText.val());
						g.setUrl(p.url, function () {
							g._selectBoxShow();
						});
					} else if (p.grid) {
						g._selectBoxShow();
						g.grid.setParm(p.textField, g.inputText.val());
						g.grid.reload();
					} else {
						g._selectBoxShow();
						p.onTextBoxKeyDown(g.inputText.val());
					}
...
(function ($)
{
	...
	$.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
		...
		_getValue: function ()
		{
			//修复获取时间为 完整日期格式数据， 返回Format格式数据  Modify By Bill He 2016-05-06
			if (this.usedDate instanceof Date) {
...
(function ($)
{
	...
	$.ligerui.controls.Filter.ligerExtend($.ligerui.core.UIComponent, {
		...
		setData: function (group, jgroup)
		{
			...
			if (group.rules) {
			    $(group.rules).each(function () {
			        ...
			        $("select.opsel", rulerow).html(g._bulidOpSelectOptionsHtml(this.type || "string", field.op));
		        ...
		        if (rowgroup) {
		            ...
		            if (value != null) {
		                groupData.rules.push({
		                    sel: sel, field: fieldName, op: op, value: value, type: type
		                });
		            }
		        }
		    });
		...
		_bulidGroupTableHtml: function (altering, allowDelete)
		{
			...
			if (allowDelete)
			    tableHtmlArr.push('<input class="l-button" type="button" value="' + p.strings['deletegroup'] + '" class="deletegroup">');
		...
		_bulidRuleRowHtml: function (fields)
		{
		    ...
		    //and or
			rowHtmlArr.push('<td class="l-filter-column">');
			rowHtmlArr.push('<select class="colsel">');
...
(function ($)
{
	...
	//***********************************************************************//
	///上传控件
	$.ligerDefaults.Form.editors.uploadify = {
		create: function (container, editParm, p) {
			var editor = $('<div class="l-uploadify"><input type="file" name="uploadify" id="uploadify" /></div>');
			var id = (p.prefixID || "") + editParm.field.name;
			if ($("#" + id).length) {
				editor = $("#" + id);
			};
			editor.find("input").attr({
				id: id,
				name: id
			});
			editor.css({ width: p.width, height: 34 });
	...
	$.ligerDefaults.Form.editors.colorPicker = {
		create: function (container, editParm, p) {
			var editor = $('<div class="l-color"><input type="text" style="width:60px;" name="color" readonly /><input id="color_d" type="text" class="input_cxcolor" readonly></div>');
			var id = (p.prefixID || "") + editParm.field.name;
			if ($("#" + id).length) {
				editor = $("#" + id);
			};
			var colorpicker = editor.find(".input_cxcolor");
			colorpicker.attr({
				id: id,
				name: id
			});
			...
			colorpicker.prev().val(colorpicker.val());
		...
		getValue: function (editor, editParm) {
			return editor.find(".input_cxcolor").val();
		},
		setValue: function (editor, value, editParm) {
			editor.find(".input_cxcolor").val(value);
		},
		resize: function (editor, width, height, editParm) {
			editor.css({
				width: width - 2
	...
	$.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render: function ()
		{
			...
			//生成标题部分
			if (p.title) {
				var jtitle = $('<div class="l-panel-header"><span class="l-panel-header-text">标题</span></div>');
				jtitle.appendTo(jform);
				$(".l-panel-header-text", jtitle).html(p.title);
			}
			//生成工具栏部分
			if (p.toolbar) {
				var jbuttons = $('<div class="l-panel-topbar"><div class="formtoolbar"></div></div>');
				jbuttons.appendTo(jform);
				$(".formtoolbar", jbuttons).ligerToolBar(p.toolbar);
			}
			...
			if (fields && fields.length)
			{
				...
				$(groups).each(function (groupIndex, group)
				{
					...
					{ 
					    ...
					    ///Modify by Bill He @ 2016-05-23
						if (p.showOpration) {

						    var _opration = "<select id='opration_" + field.name + "' " + (field.allowSelected == false ? " disabled=disabled" : "") + ">";
						    if (p.filters) {
						        $.each(p.filters, function () {
						            _opration += "<option value='" + this.value + "' " + (this.value == field.opration ? ' selected=selected' : '') + ">" + this.text + "</option>";
						        });
						    }
						    _opration += "</select>";
						    out.push('<li style="margin-right:2px;">' + _opration + '</li>');
						...
						if (field.afterContent) {
							...
							afterContent && out.push('<li style="margin-left:5px; color:#f82626;">' + afterContent + '</li>');
						}
						//append space
						if (!field.hideSpace)
						{
		...
		setDisplay: function (arg, display) {
			var fieldNames = [];
			if ($.isArray(arg)) fieldNames = arg;
			if (typeof (arg) == "string") fieldNames.push(arg);
			var g = this, p = this.options;
			setDisplayInFields(p.fields);
			if (p.tab && p.tab.items) {
				for (var i = 0; i < p.tab.items.length; i++) {
					var item = p.tab.items[i];
					if (p.tab.items.fields) {
						setVisibleInFields(item.fields, i);
					}
				}
			}
			function setDisplayInFields(fields, tabIndex) {
				$(fields).each(function (fieldIndex, field) {
					var name = field.name;
					if ($.inArray(name, fieldNames) == -1) return;
					$("#display" + name).html(display);
				});
			}
		},
		...
		getFilters: function () {
		    ...
		    $(p.fields).each(function (fieldIndex, field) {
		        var name = field.name;
		        g.data[field.name] = $("#opration_" + name).val();
		    });
		    return g.data;
		},
        ///Modify by Bill He @ 2016-05-23
		getFiltersData: function () {
		    var g = this, p = this.options;
		    var _data = g.getData();
		    var _filter = g.getFilters();
		    var _group = [];
		    for (var p in _data) {
		        _group.push({ Sel: "and", Field: p, value:( _data[p] == null ? "" : _data[p]), OP: _filter[p], Type: "string" });
		    }
		    return _group;
		},
	    ///Modify by Bill He @ 2016-05-23
		getFiltersFields: function () {
		    var g = this, p = this.options;
		    return p.fields;
		},
		...
		//动态设置下拉框数据
		setComboboxData: function (name, data) {
			var combo = liger.get(name);
			combo.clear();
			combo.set('data', data);
		},
		//提示 验证错误信息
		...
		_setTab: function (tab)
		{
			...
			for (var i = 0; i < tab.items.length; i++)
			{
				...
				if (tabItem.fields) {
					...
					var tabid = 'tab' + i + "_";
					jcontentItem.append(contentitem);
					//设置页签高度
					if (tab.height) contentitem.height(tab.height);
					else contentitem.height("auto");

					if (tabItem.height) contentitem.height(tabItem.height);

					if (tabItem.render) {
						tabItem.url = tabItem.render(this);
					}
					if (tabItem.url) {
						...
						iframe.attr("name", tabid)
						 .attr("id", tabid)
						 .attr("_src", tabItem.url);
						if (tabItem.height) {
							if (tabItem.height == "auto") {
		...
		selectTabURL: function (index, url) {
			var g = this, p = this.options;
			var jtab = $(g.element).find(".l-form-tabs:first");
			var contents = jtab.find(".ui-tabs-panel");
			contents.filter("[data-index=" + index + "]").find("iframe").attr("src", url)
		},
		selectTab: function (index)
		{
			...
			var url = contents.filter("[data-index=" + index + "]").find("iframe").attr("src");
			if (!url) {
				var _url = contents.filter("[data-index=" + index + "]").find("iframe").attr("_src");
				contents.filter("[data-index=" + index + "]").find("iframe").attr("src", _url);
			}
		},
		removeTab: function (index) {
			...
			var jtab = $(g.element).find(".l-form-tabs:first");
			var links = jtab.find(".ui-tabs-nav li"), contents = jtab.find(".ui-tabs-panel");
			links.filter("[data-index=" + index + "]").remove();
			contents.filter("[data-index=" + index + "]").remove();
		},
		destroy: function ()
		{
			try
...
(function ($)
{
	...
	$.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render: function ()
		{
			...
			gridhtmlarr.push("        <div class='l-panel-searchbar' style='display:none'><div id='formsearch' class='l-panel-searchbarinner'><div class='l-panel-button'></div></div><div class='l-panel-toggle'><div class='l-toggle-bar'></div></div></div>");
			...
			//顶部搜索栏
			g.searchbar = $(".l-panel-searchbarinner:first", g.grid);
		...
		loadData: function (loadDataParm, sourceType) {
		    ...
		    //Modify by Bill He 2016-05-16 --添加查询栏 条件参数
		    //alert(g.searchbarManager);
		    if (g.searchbarManager) {
		        var _paradata;
		        if (p.showOpration) {
		            _paradata = g.searchbarManager.getFiltersData();
		            param.push({ name: "rules", value: _paradata });
		        } else {
		            _paradata = g.searchbarManager.getData();
		            for (i in _paradata) {
		                if (_paradata[i] != null && _paradata[i] != "") {
		                    //g.setParm(i, _paradata[i]);
		                    param.push({ name: i, value: _paradata[i] });
		                }
		            }
		        }
		    }
		    //Modify by Bill He 2016-05-16 --添加高级查询栏 条件参数
		    if (p.filterData) {
		        //alert(JSON.stringify(p.filterData));
		        param.push({ name: "rules", value: p.filterData });
		    }
		    ...
		    if (p.dataType == "local") {
		        ...
		        //alert(JSON.stringify(g._searchData()));
		...
		loadServerData: function (param, clause, sourceType)
		{
		    ...
		    //[15:18:12] Wade Cai @ Dimerco CRP SHA: 
			var myData = p.data || {};
			$.each(param, function (i, o) {
			    myData[o.name] = o.value;
			});
			param = myData;

		    //$("body").append(JSON.stringify(param));
			//alert(url);
			var ajaxOptions = {
				type: p.method,
				url: url,
				...
				success: function (data)
				{
				    //modify by Bill He 2016-05-04
				    //alert(JSON.stringify(data));
					//if (data.ReturnData)
						//data = { Rows: eval(data.ReturnData), Total: data.RecordCount };
		...
		*/
		...
		_initBuildSearch: function () {
			var g = this, p = this.options;
			if (p.searchbar) {
				if (p.searchbar.fields.length == 0) return;
				g.searchbar.parent().show();
				if ($.fn.ligerForm) {
					g.searchbarManager = g.searchbar.ligerForm(p.searchbar);
					//设置回车事件查询
					...
					//设置默认值
					//设置只读
					$.each(p.searchbar.fields, function () {
						switch (this.type) {
							case "select":
								$("input[name=" + this.name + "Text]").bind("change", function () {
								    g.loadData();
								});
								break;
							case "date":
								$("input[name=" + this.name + "]").bind("change", function () {
								    g.loadData();
								});
								break;
							default:
								$("input[name=" + this.name + "]").bind("blur", function () {
								    g.loadData();
								});
								break;
						}
						if (this.readonly == true) {
							liger.get(this.name).setDisabled();
						}
						if (this.value != null) {
							var _value = {};
							_value[this.name] = this.value;
							g.searchbarManager.setData(_value);
						}
					});
					var searchbutton = g.searchbar.parent().find(".l-panel-button");
					var searchform = g.searchbar.parent().find(".l-form-container");
					searchbutton.appendTo(g.searchbar);
					var hidebutton = g.searchbar.parent().find(".l-panel-toggle").find(".l-toggle-bar");
					//设置查询栏点击事件
					searchbutton.ligerToolBar({
						items: [{
							text: $.ligerDefaults.GridString.searchMessage, click: function (item) {
							    g.loadData();
							}, icon: 'search'
						},
						{
							text: $.ligerDefaults.GridString.resetMessage, click: function () {
								var _paradata = g.searchbarManager.getData();
								for (i in _paradata) {
									if (_paradata[i] != null || _paradata[i] != "") {
										_paradata[i] = "";
									}
								}
								g.searchbarManager.setData(_paradata);
								g.loadData();
							}, icon: 'refresh'
						}]
					});
					//var margin = (g.searchbar.height() - 82) / 2;
					//searchform.css({ width: g.searchbar.width() - 90, float: 'left' });
					//searchbutton.css({ float: 'left', width: 70, height: 60, 'margin-top': margin });
					if (p.searchbuttonshow == false) {
						searchbutton.hide();
					}
					hidebutton.click(function () {
						var _ishide = $(this).attr("hide");
						$(this).attr("title", $.ligerDefaults.GridString.hideMessage);
						if (_ishide == "true") {
							g.searchbar.show();
							...
							g.searchbar.hide();
							$(this).attr("hide", "true");
							$(this).attr("title", $.ligerDefaults.GridString.showMessage);
						}
						g._initHeight();
					});

					//原语句不知道为什么, toolbar的父div高度为0. 导致样式有问题. 
					if (g.searchbar.height() == 0)
						g.searchbar.parent().height("auto");
					else
						g.searchbar.parent().height("auto");
				}
			}
			else {
				g.searchbar.parent().remove();
			}
		},
		...
		_initColumns: function ()
		{
			...
			//行工具栏
			if (p.rowtoolbar) {
				if (p.rowtoolbar.items) {
					var _width = (p.rowtoolbar.items.length * 22) + 6;
					var col = { isrowtoolbar: true, issystem: true, width: _width, frozen: true };
					setColumn(col, 1, -1, lastid);
					lastid = col['__id'];
				}
			}
		...
		_createHeaderCell: function (column)
		{
			...
			if (column.isrowtoolbar) {
				jcell.addClass("l-grid-hd-cell-rowtoolbar");
				jcell.html("<div class='l-grid-hd-cell-inner'></div>");
			}
		...
		_initHeight: function ()
		{
			...
			//}
			if (p.height == 'auto') {
				var _winHeight = $(window).height() == 0 ? 380 : $(window).height()-4;
				var _topbarH = p.toolbar == null ? 2 : g.topbar.parent().height() + 2;//工具栏高度
				var _headerH = p.title == null ? 2 : g.header.height() + 2;//标题栏高度
				var _toolbarH = p.usePager == false ? 1 : g.toolbar.height() + 2;//底部工具栏高度
				var _gridheaderH = g.gridheader.css("display") == "none" ? 0 : g.gridheader.height() + 2;//表头高度
				var _searchbarH = p.searchbar == null ? 2 : g.searchbar.height() + 2;//查询栏高度
				var _height = _winHeight - _headerH - _toolbarH - _gridheaderH - _topbarH - _searchbarH + p.heightDiff;
				//alert(_headerH + _toolbarH + _gridheaderH + _topbarH + _searchbarH + p.heightDiff);
				g.gridbody.height(_height);
				g.f.gridbody.height(_height);
			}
			if (p.width)
			{
		...
		_showData: function (sourceType)
		{
			...
			if (p.rowtoolbar) {
				if (p.rowtoolbar.items) {
					$(".l-grid-row-cell-rowtoolbar .l-grid-row-cell-inner").each(function () {
						var _rowid = g._getRowIdByDomId($(this).parent().attr("id"));
						$(this).find(".l-cell-button").each(function (i) {
							$(this).click(function () {
								//获取生成事件
								$.each(g.currentData.Rows, function () {
									if (this["__id"] == _rowid) p.rowtoolbar.items[i].click(this);
								});
							});
						});
					});
		...
		_getHtmlFromData: function (data, frozen)
		{
			...
			for (var i = 0, l = data.length; i < l; i++)
			{
				...
				if (p.rowtoolbar) {
					if (p.rowtoolbar.items) {
						$.each(p.rowtoolbar.items, function () {
							_toolhtml += "<div class='l-cell-button' style='margin-top:" + (p.rowHeight-20)/2 + "px;' title='" + this.text + "'><div class='l-icon l-icon-" + this.icon + "'></div></div>";
						});
					}
				}
				...
				$(g.columns).each(function (columnindex, column)
				{
					...
					//如果是行工具栏(系统列)
					if (this.isrowtoolbar) {
						gridhtmlarr.push(' class="l-grid-row-cell l-grid-row-cell-rowtoolbar" style="width:' + this.width + 'px"><div class="l-grid-row-cell-inner"');
						if (p.fixedCellHeight)
							gridhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" ');
						else
							gridhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px; " ');

						gridhtmlarr.push('>' + _toolhtml + '</div></td>');
						return;
					}
					//如果是复选框(系统列)
					if (this.ischeckbox)
		...
		_applyEditor: function (obj)
		{
			...
			//alert(JSON.stringify(column));  获取键值 消失了
			...
			if (column.editor.type && p.editors[column.editor.type])
			{
				...
				var editor = p.editors[column.editor.type],
					...
					container = $("<div class='l-grid-editor'></div>").appendTo(jcell),
					left = 0,
					top = 0,
					pc = jcell.position(),
		...
		_getTotalSummaryHtml: function (data, classCssName, frozen)
		{
			...
			$(g.columns).each(function (columnindex, column)
			{
				...
				//如果是行工具栏(系统列)
				if (this.isrowtoolbar) {
					totalsummaryArr.push('<td class="l-grid-totalsummary-cell l-grid-totalsummary-cell-rowtoolbar" style="width:' + this.width + 'px"><div>&nbsp;</div></td>');
					return;
				}
				//如果是复选框(系统列)
				if (this.ischeckbox)
		...
		showFilter: function () {
		    ...
		    g.filter = filtercontainer.ligerFilter({ fields: getFields() });
		    //g.filter.addRule($(g.filter.element.firstChild));
		    if (p.searchbar) {
		        var _group = { rules: [] };
		        $.each(p.searchbar.fields, function () {
		            var _rule = { field: this.name, op: this.op, value: this.value, type: this.type };
		            _group.rules.push(_rule);
		        });
		        //g.filter.setData(_group);
		    }
...
(function ($)
{
	...
	$.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
		...
		_getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand)
		{
			...
			//去除重复数据。。。。LigerUI Bug(连续几次点击生成树，子节点会有重复数据)
			var tmp = {}, b = [], a = data;
			for (var i = 0; i < a.length; i++) {
				var _d = a[i];
				if (b.length == 0) b.push(_d);
				else {
					var _count = 0;
					$.each(b, function () {
						if (this == _d) _count++;
					})
					if (_count == 0) b.push(_d);
				}
			};
			data = b;
			//////////////////////////////////////////////////////////////////////////
			...
			for (var i = 0; i < data.length; i++)
			{
				...
				if (o.treedataindex != undefined) {
					...
					//Update By Bill He 添加传入参数 数值
					treehtmlarr.push('treedataval="' + o[p.idFieldName] + '" ');
				}
				if (isExpandCurrent)
...
* jQuery ligerUI 1.3.2
* 
* Author Bill He  ligerShortcut
* 
*/
(function ($) {

	$.fn.ligerShortcut = function (options) {
		return $.ligerui.run.call(this, "ligerShortcut", arguments);
	};
	$.fn.ligerGetShortcutManager = function () {
		return $.ligerui.run.call(this, "ligerGetShortcutManager", arguments);
	};
	$.ligerDefaults.Shortcut = {
		style: "link",
		height:"auto",
		float: "left",
		click: null,
	...
	$.ligerMethos.Shortcut = {};

	$.ligerui.controls.Shortcut = function (element, options) {
		$.ligerui.controls.Shortcut.base.constructor.call(this, element, options);
	};
	$.ligerui.controls.Shortcut.ligerExtend($.ligerui.core.UIComponent, {
		__getType: function () {
			return 'Shortcut';
		},
		__idPrev: function () {
		...
		_extendMethods: function () {
			return $.ligerMethos.Shortcut;
		},
		_render: function () {
			var g = this, p = this.options;
			g.shortcut = $(g.element);
			if (p.height == "auto")
				g.shortcut.height($(window).height() + p.heightDiff);
			else
				g.shortcut.height(p.height + p.heightDiff);
			
			if (p.style == "menu") g.shortcut.parent().addClass("l-shortcut-background");
			$.each(p.data, function () {
				if (p.style == "ico") {
					g.shortcut.append('<div class="l-shortcut" rel="' + this.url + '" parent="' + this.parent + '" key="' + this.key + '" idx="' + this.id + '"><div class="l-shortcut-ico"><img _src="' + this.icon + '"></div><div class="l-shortcut-text">' + this.text + '</div></div>');
				} else if (p.style == "menu") {
					g.shortcut.append('<div class="l-shortcut l-shortcut-menu l-shortcut-background" rel="' + this.url + '" parent="' + this.parent + '" key="' + this.key + '" idx="' + this.id + '"><div></div><div class="l-list-text">' + this.text + '</div></div>');
				} else {
					g.shortcut.append('<div class="l-shortcut l-shortcut-list" rel="' + this.url + '" parent="' + this.parent + '" key="' + this.key + '" idx="' + this.id + '"><div class="l-list-ico"></div><div class="l-list-text">' + this.text + '</div></div>');
				}
			})
			g.shortcut.find(".l-shortcut").each(function () {
				$(this).hover(function () {
					if ($(this).disabled) return;
					$(this).addClass("l-shortcut-over");
				}, function () {
					if ($(this).disabled) return;
					$(this).removeClass("l-shortcut-over");
				});
				if (p.style == "link") {
					var _backimg = $(this).find(".l-shortcut-ico").find("img").attr("_src");
					$(this).find(".l-shortcut-ico").empty()
					$(this).find(".l-shortcut-ico").css({ "background-image": "url('resources/icons/" + _backimg + "')" });
				}
				$(this).click(function () {
					g.shortcut.find(".l-shortcut").each(function () {
						$(this).removeClass("l-shortcut-selected ");
						if (p.style == "menu") {
							$(this).addClass("l-shortcut-background")
						}
					});
					$(this).removeClass("l-shortcut-background").addClass("l-shortcut-selected");
					var node = { target: $(this), data: { url: $(this).attr("rel"), text: $(this).find("div").eq(1).html(), key: $(this).attr("key"), parent: $(this).attr("parent"), id: $(this).attr("idx") } }
					if (!p.disabled) {
						p.click(node);
					}
				});
				$(this).dblclick(function () {
					...
					$(this).addClass("l-shortcut-selected");
					var node = { target: $(this), data: { url: $(this).attr("rel"), text: $(this).find("div").eq(1).html(), key: $(this).attr("key"), parent: $(this).attr("parent"), id: $(this).attr("idx") } }
					if (!p.disabled) {
						p.dbclick(node);
					}
		...
		_setIcon: function (url) {
			var g = this;
			if (!url) {
				g.shortcut.removeClass("l-button-hasicon");
				g.shortcut.find('img').remove();
			} else {
				g.shortcut.addClass("l-button-hasicon");
				g.shortcut.append('<img src="' + url + '" />');
			}
		},
		_setEnabled: function (value) {
			if (value)
				this.shortcut.removeClass("l-button-disabled");
		},
		_setDisabled: function (value) {
			if (value) {
				this.shortcut.addClass("l-button-disabled");
				this.shortcut.disabled = true;
			} else {
				this.shortcut.removeClass("l-button-disabled");
				this.shortcut.disabled = false;
			}
		},
		_setWidth: function (value) {
			this.shortcut.width(value);
		},
		setEnabled: function () {
			this.set('disabled', false);
		},
		...
		getNode: function () {
			var g = this, p = this.options;
			var _selected = g.shortcut.find(".l-shortcut-selected");
			if (_selected.length == 0) return null;
			else
				return { target: _selected, data: { url: _selected.attr("rel"), text: _selected.find("div").eq(1).html(), key: _selected.attr("key"), parent: _selected.attr("parent"), id: _selected.attr("idx") } }
		}
	});
})(jQuery);