...
(function ($)
{
	...
	$.ligerui.controls.Filter.ligerExtend($.ligerui.core.UIComponent, {
		...
		_bulidGroupTableHtml: function (altering, allowDelete)
		{
            ...
            //Steve Added 20160519//////////////////////////////////////////
			if (g.id == "maingrid_filtercontainer") {
	...
	$.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
		...
		loadData: function (loadDataParm, sourceType) {
		    ...
		    if (p.filterData) {
		        ...
		        param.push({ name: "filter", value: JSON.stringify(p.filterData) });
		    }
		        ...
		        buttons: [
					{
					    text: '确定', onclick: function (item, dialog) {
					        p.filterData = filter.getData();
					        g.loadData();
					        //Steve Added 20160519
                            // save to DB
					        $("#txtGroup").val($.ligerui.toJSON(p.filterData));
					        var temp = $.ligerui.toJSON(p.filterData);
					        // set filter with ID="filter"
					        var advfilter = $("#filter").ligerFilter({ fields: fields });
					        advfilter.setData(p.filterData);
                            //////////////////////
					        dialog.hide();
					    }
					},
